...
class EquipeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Equipe entity")
        ...
        .repeat(2) {
            ...
            .get("/services/codegeneral/api/equipes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new equipe")
            .post("/services/codegeneral/api/equipes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "refEquipe":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/services/codegeneral${new_equipe_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created equipe")
            .delete("/services/codegeneral${new_equipe_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
